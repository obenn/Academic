...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize){
		...
		for (int n = interval;  n <= maxSize; n += interval) {
			...
			for (int i = 1; i <= x; i++) {
				...
				double runTime = (double) finishedTime - initialTime;

				if (runTime > largestRunTime) {
					largestRunTime = runTime;
				}
			}
			double t = largestRunTime;
			System.out.println(
							" n: "   		+ n +
							"\n t: "   		+ t +
							"\n t/n: " 		+ t / n +
							"\n t/n*n "		+ (double) t/(n*n)*1000 +
							"\n t/nlog(n) "	+ (double) (t/Math.log(n)/Math.log(2)) +
							"\n");
			largestRunTime = 0;
		}
	}
	...
	public static long unique1Runtime(int n){
		...
		return finishedTime - initialTime;
	}
