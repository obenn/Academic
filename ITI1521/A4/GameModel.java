...
public class GameModel implements Cloneable, Serializable {
	...
	/**
	 * Booleans used to track user options for gameplay style
	 */
    ...
    public void reset() {
    	...
    	numberOfSteps = -1; // -1 Represnts the select color step
        numberCaptured = 0;
    }
   ...
   /**
	...
	/**
	 * Override clone method to support deep copy
	 */
	public GameModel clone() {
		try {
			GameModel that = (GameModel) super.clone();
			that.model = this.model.clone();
			for (int i = 0; i < this.model.length; i++) {
				that.model[i] = this.model[i].clone();
			}
			for(int i = 0; i < sizeOfGame; i++){
			   	for(int j = 0; j < sizeOfGame; j++){
					that.model[i][j] = this.model[i][j].clone();
				}
			}
			return that;
